...
public class UserInterfaceConsole {
	...
	private void welcomeVolunteer(String theUserName, Volunteer theVolunteer) {
		...
		System.out.println("You are signed in as a Volunteer");
		displayDate();
	...
	private void welcomeParkManager(String theUserName, ParkManager theParkManager) {
		displaySeperator();
		displayGreeting(theUserName);
		System.out.println("You are signed in as a Park Manager");
		displayDate();
	...
	private boolean tryToAddJob(Job theJob) {
		...
		int result = myJobCoordinator.checkBusinessRules(theJob, 
				(ParkManager) mySystemCoordinator.getUser(myUserName));
		if (result == 1) {
			...
			((ParkManager) mySystemCoordinator.getUser(myUserName)).addCreatedJob(theJob);
			canAddJob = true;
		}
		return canAddJob;
	}
